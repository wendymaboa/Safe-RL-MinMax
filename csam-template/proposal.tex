...
\newcommand{\name}{Wendy Maboa}
\newcommand{\studentNumber}{2541693}
\newcommand{\supervisor}{Geraud Nangue Tasse}
\proposal
\csmsc
...
\aideclareEditing
\aideclareWriting
\aideclareExtra{Elicit}{Literature survey}
...
resource-intensive components like the Lagrange multiplier. Moreover, models fine-tuned with these approaches can still
...
Lagrange multiplier. The proposed experiments evaluate the approach on GPT-2 small and a
small Qwen model (exact size in the 0.5 to 3B parameter range, subject to available
compute), trained on BeaverTails with Detoxify as the unsafe-state detector, against a
KL-penalty baseline on BeaverTails-Evaluation. This research provides an accessible route
to improving safety in PPO-based LLM fine-tuning without additional annotation or
...
Because Algorithm~1 only acts when the unsafe-state detector fires, detector quality is
expected to be the primary variable affecting coverage. This motivates a two-phase research
programme. In Phase 1, Detoxify \citep{hanu2020detoxify} serves as the unsafe-state
detector, providing a fast, reproducible safety signal for harm categories that manifest as
surface-level toxic language. In Phase 2, Detoxify is replaced by an activation-based probe
...
Chapter~\ref{ch:research-questions} states the research questions and hypotheses.
...
This chapter states the problem addressed by the research and the questions and hypotheses
that structure the experimental programme.
...
This chapter describes the full experimental design, implementation, evaluation strategy,
ablation studies, and limitations.

\section{Experimental Methodology}
\label{sec:methodology}

This chapter details the experimental methodology. PPO is used for all fine-tuning runs,
...
such as base model, fine-tuning strategy, and detector threshold. The methodology is structured around four components: data collection (Section~\ref{sec:data-collection}),
...
\textbf{Statistical scope.} The full experimental programme will use multiple seeds per
condition so that reported harm-rate differences can be tested for significance rather than
...
relative to the project start in May 2026 and the standard two-year MSc duration.
Cluster-dependent phases (Qwen model scale, HarmBench evaluation) are scheduled after
cluster access is confirmed.
...
\textbf{Minimum bar.} Minmax training reduces harm rate relative to the KL-penalty
baseline on BeaverTails-Evaluation. Extended training shows stable or improving safety
performance. Ablation studies document sensitivity to threshold, KL anchor, fine-tuning
method, and base model. Per-category analysis attributes coverage gaps (e.g.\ self-harm
under Detoxify) to detector quality.
...
Phase 2 is motivated by the expected coverage limits of surface-level detectors:
replacing Detoxify with an activation probe trained to detect unsafe internal states before
the output token is produced. Based on \citet{rateike2023}'s finding that hallucination
detection is the most tractable initial subset, the probe will first target factually
incorrect and internally inconsistent outputs.
...
The research direction follows from the structure of Algorithm~1: the penalty mechanism
acts only when the unsafe-state detector fires, so detector quality is the primary variable
modulating coverage. The two-phase architecture, moving from surface-level toxicity
detection to activation-based safety signals, is designed to address this directly. The
planned ablation programme, varying base model, fine-tuning strategy, and detector
