...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog='deepspeed --module safe_rlhf.algorithms.ppo_cost_gate',
        description='Train language model using PPO with a threshold-gated fixed cost penalty.',
    ...
    training_parser.add_argument(
        ...
        help=(
            ...
            '--penalty_magnitude for that step. Default 0.0 matches the Stage 4 finding '
            'that beaver-7b-unified-cost assigns negative cost to every benign response '
            'observed and strongly positive cost to harmful ones — so 0 is a principled '
    ...
    training_parser.add_argument(
        ...
        default=-2.0,
        help=(
            'The fixed reward value assigned to any response with cost > cost_threshold, '
            'replacing its reward-model score outright rather than blending with it. '
            'This is Run B: the control for Run C (Minmax), which uses the identical gate '
            'but replaces this constant with a self-calibrated V_MIN - V_MAX bound.'
        ),
    )
    training_parser.add_argument(
        '--use_torch_adam',
...
def main() -> None:
    ...
    trainer = PPOCostGateTrainer(args, ds_train_config, ds_eval_config)
    trainer.train()
    trainer.save()
