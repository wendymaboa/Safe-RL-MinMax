...
# ==============================================================================
"""The main training script to train PPO with a threshold-gated Minmax cost penalty."""
...
from safe_rlhf.algorithms.ppo_cost_minmax.trainer import PPOCostMinmaxTrainer
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog='deepspeed --module safe_rlhf.algorithms.ppo_cost_minmax',
        description='Train language model using PPO with a threshold-gated Minmax cost penalty.',
    ...
    training_parser.add_argument(
        ...
        default=0.0,
        help=(
            'A response with cost strictly above this value has its reward replaced by '
            'R_unsafe = V_MIN - V_MAX for that step. Default 0.0 matches Run B and the '
            'Stage 4 finding that beaver-7b-unified-cost assigns negative cost to every '
            'benign response observed and strongly positive cost to harmful ones.'
        ),
    )
    training_parser.add_argument(
        '--penalty_floor',
        ...
        default=-50.0,
        help=(
            'Hard lower bound on R_unsafe = V_MIN - V_MAX. Default −50 matches '
            '--clip_range_score so the floor is an engineering backstop, not the '
            'comparison point: Run B\'s fixed penalty is −2.0, and the thesis question '
            'is whether the self-calibrated bound moves past that. Phase 1 floored at '
            '−2.0 because Detoxify rewards were bounded to [−1, 1]; do not reintroduce '
            'that floor here or the saturation finding cannot be re-tested.'
        ),
    )
    training_parser.add_argument(
        '--bound_source',
        ...
        help=(
            'What expands V_MIN / V_MAX each step. "reward" uses reward-model end scores '
            'only (Phase 1\'s stable default). "reward_and_value" also folds in the '
            'terminal critic estimate after --critic_warmup_steps.'
        ),
    )
    training_parser.add_argument(
        '--critic_warmup_steps',
        ...
        default=100,
        help=(
            'Steps before critic values enter the bound update when '
            '--bound_source=reward_and_value. Ignored for bound_source=reward.'
        ),
    )
    training_parser.add_argument(
        '--use_torch_adam',
...
def main() -> None:
    ...
    trainer = PPOCostMinmaxTrainer(args, ds_train_config, ds_eval_config)
    trainer.train()
    trainer.save()
